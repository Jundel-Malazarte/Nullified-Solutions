    ...
    if ($email === '' || $password === '') {
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            if (($user['role'] ?? 'customer') !== 'admin') {
                ...
                $error = 'This admin account is not active. Please contact support.';
            } else {
...
<html lang="en">
  <head>
    ...
    <title>Admin Log In | Nullified Solutions</title>
    ...
    <main class="account-main">
      <section class="account-shell" aria-labelledby="login-title">
        <div class="account-intro">
          ...
          <h1 id="login-title">Secure admin portal.</h1>
          <p>Sign in to manage bookings, pricing, accounts, and store inventory from one dashboard.</p>
        </div>

        <div class="account-form-wrap">
          <p class="eyebrow">NULLIFIED SOLUTIONS ADMIN</p>
          <h2>Log in to admin account</h2>
          <p>Enter your admin details to continue.</p>
          ...
          <form class="account-form" method="post" action="admin_login.php" id="loginForm">
            <label>Email address
